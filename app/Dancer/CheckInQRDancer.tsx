...
import { dancerCompleteWork } from '../../service/api';

const ORANGE2 = '#FF7A00';

export default function CheckInQRDancer() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      setSubmitting(true);
      // Expecting QR data as JSON string with booking information
      const parsed = JSON.parse(scannedPayload.data);
      const bookingId = Number(parsed.bookingId);

      if (!bookingId) {
        throw new Error('Không tìm thấy mã đơn đặt');
      }

      await dancerCompleteWork(bookingId);
      setModalVisible(false);
      router.back();
    } catch (e: any) {
      // keep modal open to let user retry or cancel
      console.error('Error completing work:', e);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <View style={styles.screen}>
      ...
      {modalVisible && scannedPayload && (() => {
        ...
        const bookingDate = parsedData?.bookingDate || '';
        ...
        return (
          <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
              <Text style={styles.modalTitle}>Bạn có muốn hoàn thành công việc này?</Text>
                ...
                <View style={styles.modalContent}>
                  ...
                  {bookingId ? (
                    <View style={styles.modalInfoRow}>
                      ...
                      <Text style={styles.modalValue}>#{bookingId}</Text>
                    </View>
                  ) : null}
                </View>
              )}
