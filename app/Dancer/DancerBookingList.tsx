...
import { dancerGetBooking } from '../../service/api';
...
export default function DancerBookingList() {
    ...
    try {
      ...
      setError(
        e?.response?.data?.message ||
          e?.message ||
          'Lỗi khi tải danh sách đặt lịch',
      );
    } finally {
      setLoading(false);
  ...
  const openDetail = (booking: any) => {
    ...
    //     params: { booking: serialized },
    //   });
    // } catch {
    //   router.push({
    //     pathname: '/BookingDetail',
    //     params: { booking } as any,
    //   });
    // }
  };
        ...
        <View style={styles.feedbackRow}>
          ...
          {hasFeedback && (
            <Text style={styles.feedbackCount}>
              {booking.feedbacks.length} đánh giá
            </Text>
          )}
        </View>
      )}
